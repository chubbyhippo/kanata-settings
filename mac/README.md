...
| You want | Do this |
|---|---|
| Copy / Paste / Cut / Undo | hold LCmd thumb (NUM) + `x` / `v` / `c` / `z` · or combos `x·c` / `c·v` / `z·x` |
...
| Delete word forward / back | NAV (hold `x`/`.`): `v` = forward · `z` = back (Opt+Del / Opt+Bksp) |
...
+-------+-------+-------+-------+-------+    +-------+-------+-------+-------+-------+
| Wrd←  | Left  | Down  | Right | Wrd→  |    |       | osCtl | osSft | osOpt | osCmd |
+-------+-------+-------+-------+-------+    +-------+-------+-------+-------+-------+
...
`Bsp` Backspace · `Pst` Paste · `⌘←`/`⌘→` line start / end · `Num` Num Word · `Plain` plain mode · `NoCmb` all-combos toggle.
